   ...
   <div class="bg-info-subtle text-info-emphasis py-5">
    ...
    <div class="container bg-white p-5 rounded-4 shadow">
      ...
      <div class="row g-3 align-items-center">
        ...
        <div class="col-auto">
          <button type="button" class="btn btn-outline-warning"  @click="searchDepartmentById"> Search </button>
        </div>
      </div>

      <form class="row mb-3 needs-validation" @submit.prevent="handleSubmit">
        <div class="col-sm-25">
          <label for="inputDepartmentName" class="form-label">Department Name</label>
          <input type="text" class="form-control" id="inputDepartmenttName" required
        ...
        <div class="col-sm-25">
          ...
          <input type="text" class="form-control" id="inputHODName" required v-model="formData.HODName" />
          {{ formData.HODName }}
        </div>
        <div class="col-md-2">
...
export default {
  ...
  methods: {
    async handleSubmit() {
      let url = `http://localhost:3001/departments/${this.departmentId}`
...
<style scoped>
.bg-info-subtle {
  background-color: #eeebbf !important;
  /* light voilet */
  min-height: 100vh;
}
